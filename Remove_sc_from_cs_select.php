  ...
  <tr>
    <td align="center" valign="top" style="padding:5px;"><table width="738"  border="0" cellpadding="0" cellspacing="0">
      <tr>
        <td height="148" valign="top"><table width="738"  border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="753" height="44" background="Images/remove_sc.jpg">&nbsp; </td>
          </tr>
          <tr>
    <table align="center" width="100%" border="bordered" > 
    <form   name= "kk" method="post" enctype="multipart/form-data" action="Remove_sc_from_cs_ok.php" >      
    <tr align="center">
    <?php
	if(0==$_SESSION[gid]){
	?>
      ...
      <?php
            ...
            $i=1;
            do{
				$sql_link=mysql_query("select * from case_study as c INNER JOIN user as d on c.owner = d.userId where id = $info[cs_id] order by id");
      ...
      <tr>
        ...
        <td style="padding:5px;" align="center">&nbsp;<?php echo $info_link[username];?></td>
	...
	?>
      ...
      <?php
            ...
            $info_sc_info=mysql_fetch_array($sql);
            $i=1;
            do{
            ?>
      <tr>
        <td height="25" align="center"><?php echo $i;?></td>
			 <?php
            $cs_sql=mysql_query("select * from case_study as c INNER JOIN case_study_details as b on c.id=b.cs_id where '$info_sc_info[scenariosId]'=b.scenario_id");
            $info=mysql_fetch_array($cs_sql);
                ?>
                <td style="padding:5px;" align="center"><?php echo $info[name];?>
                <td style="padding:5px;"><label><input name="sel_sc" type="checkbox" value="<?php echo $info_sc_info[scenariosId];?>" onClick=""> &nbsp;<?php echo $info_sc_info[scenarios_name];?></label>
        <td style="padding:5px;">&nbsp;<?php echo $info_sc_info[scenarios_description];?></td>
      ...
      <?php 
         $i=$i+1;
         }while($info_sc_info=mysql_fetch_array($sql));
        ?>
        </td>
  </tr>
<?php }?>
